...
sudo docker build -f "Dockerfile" -t agungajin19/container:FE-$TRAVIS_BUILD_NUMBER  --label "Maintaner  Agung Aji Nugroho <nugroho@alterra.id>" .
# push apps image to docker hub
sudo docker push agungajin19/container:FE-$TRAVIS_BUILD_NUMBER
...
kubectl -n alta4 --kubeconfig kubeconfig set image deployment/${APP_K8S_DEPLOYMENT_NAME} ${APP_K8S_CONTAINER_NAME}=agungajin19/container:FE-${TRAVIS_BUILD_NUMBER}